...
- **控制条浮在画面上、不占位置。** 进度条、倍速、音量都浮在画面底部（窗口模式和全屏一个样）：
  鼠标不动约 2.6 秒就淡出，动一下鼠标、碰一下屏幕或按一下键再浮回来；暂停时一直留着，
  鼠标停在控制条上时也不会淡出。点「⛶ 全屏」画面铺满整个屏幕，按 Esc 退出。
- **浮层画不出来时的退路。** 顶栏有「控制条」按钮，可在「浮层」和「停靠」之间切换：
  停靠 = 控制条回到画面下方常显（早期版本那样），浮层在某些显卡/浏览器上不显示时用这个。
  旁边的状态胶囊会实时显示「控制条 显示中 / 已隐藏 / 停靠常显」——看不到控制条时先看它：
  显示「显示中」却看不到，说明是浏览器没把浮层画出来（点左边按钮切「停靠」即可）；
  显示「已隐藏」却一直在晃鼠标，那才是浮现逻辑出了问题。
  顶栏「诊断」（等价于地址后加 `?debug=1`）显示实时状态：界面版本、指针事件计数、
  控制条 opacity/pointer-events/位置——看不出控制条时打开它，一眼就知道卡在哪一步。
