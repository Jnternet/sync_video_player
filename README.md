...
- **控制条平时收起，晃鼠标就滑出来，画面不会跟着跳。** 默认是「滑出」：进度条、倍速、音量贴在
  画面下方，鼠标一动就从底下滑出，静止约 2.6 秒再收起，暂停时一直留着。**全屏时控制条的
  位置是常驻预留的**——画面尺寸与位置固定不变，调进度时画面绝不会上下跳；窗口模式下收起时
  高度为 0（不占位置）。点「⛶ 全屏」画面铺满屏幕，按 Esc 退出。
- **想要 bilibili 那种叠在画面上的效果**：顶栏「控制条」按钮切到「浮层」，这时画面完全不动、
  控制条叠在画面底部。注意有些显卡/浏览器不肯把网页浮层画在视频上面（表现是「点得到、看不见」）
  ——切过去看不到就直接切回「滑出」，那条路不依赖浮层，任何环境都能显示。旁边的状态胶囊会
  实时显示「控制条 显示中 / 已收起 / 已隐藏」。
  顶栏「诊断」（等价于地址后加 `?debug=1`）显示实时状态：界面版本、指针事件计数、
  控制条 opacity/pointer-events/位置和命中测试——看不出控制条时打开它，一眼就知道卡在哪一步。
