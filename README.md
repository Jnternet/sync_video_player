...
- **控制条平时收起，晃鼠标就从画面下方滑出来。** 默认是「滑出」：进度条、倍速、音量贴在画面
  下方，收起时高度为 0（一点位置都不占），鼠标一动就从底下滑出，静止约 2.6 秒再收起；
  暂停时一直留着。点「⛶ 全屏」画面铺满屏幕，按 Esc 退出。
- **想要 bilibili 那种叠在画面上的效果**：顶栏「控制条」按钮可以切到「浮层」。注意有些
  显卡/浏览器不肯把网页浮层画在视频上面（表现是「点得到、看不见」）——切过去看不到就直接
  切回「滑出」，滑出这条路不依赖浮层，任何环境都能显示。旁边的状态胶囊会实时显示
  「控制条 显示中 / 已收起 / 已隐藏」，看不到控制条时先看它。
