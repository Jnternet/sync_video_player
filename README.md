...
- **浮层看不见时切「滑出」。** 有些显卡/浏览器会把视频送进硬件叠加层，网页浮层会被它盖住
  （表现是「点得到、看不见」）。顶栏「控制条」按钮可以切到「滑出」：控制条挪到画面下方、
  不叠在画面上，任何环境都画得出来——代价是它会占一条高度（全屏时该高度常驻预留，画面仍然
  不会动，只是整体略小一点）。旁边的状态胶囊实时显示「控制条 显示中 / 已隐藏 / 已收起」。
