...
- **键盘快捷键。** 顶栏「⌨ 快捷键」或按 `Q` 随时调出提示面板：`F` 全屏 / 退出全屏；
  `M` 静音；`W` / `S`（或 `↑` / `↓`）音量 ±10%，音量 ≤10% 时改成每次 ±2% 好微调；
  `A` / `D`（或 `←` / `→`）后退 / 前进 5 秒；按住 `D` / `→` 超过 0.2 秒进入 2 倍速快进，
  松开回到原速。跳转和快进都跟播放/暂停、拖进度条一样属于控制信息，会同步给房间里所有人
  （快进是全场一起 2 倍速，松手一起回原速；别人中途改过倍速就不会被覆盖）；
  音量与静音只在本机生效，一个字节都不发出去。在输入框里打字时快捷键自动让位。
