...
- `check-ui.mjs`：`app.js` 用到的 id/class 必须都在 `index.html` 里，且 id 不重复
  ...
  必须同时保留两条路：浮层（`.ctl-float`，默认；绝对定位 + `translateZ(0)` + `backdrop-filter`，
  再把视频用 `opacity<1 + border-radius + translateZ(0)` 从硬件叠加层里踢出来，否则浮层会被
  视频叠加层盖住、表现是「点得到、看不见」）和滑出兜底（文档流里「收起时 `max-height:0`、
  有活动时展开」，**全屏时必须常驻预留控制条位置**——高度固定、只淡入淡出，画面尺寸位置不能变，
...
- `check-fullscreen.mjs`：同样加载真实的 `web/app.js`，用假定时器推进时间，断言全屏控制条
  ...
  浮层、切回滑出」。
- `check-shortcuts.mjs`：同样加载真实的 `web/app.js`，断言快捷键：`Q` 开关提示面板
  （顶栏按钮 / 关闭按钮 / 点暗背景 / `Esc` 都能关，面板开着时控制条不淡出）、`F` 全屏的是
  `.stage`、`M` 静音与 `W` / `S` / `↑` / `↓` 音量 ±10%（音量 ≤10% 时 ±2%，到 0/100 停住，
  两者都一个请求都不发）、`A` / `D` / `←` / `→` 后退/前进 5 秒且**跟拖进度条一样把 `seek`
  发给房间**、
  按住 `D` / `→` 不到 0.2 秒算单击（+5 秒）、超过 0.2 秒进 2 倍速并把**倍速发给房间**，
  松手/失焦/关页面（`sendBeacon` 兜底）都把原来的倍速还回去——房间 1 倍速时就是 1 倍，
  期间别人改过倍速则不覆盖别人的选择；长按不发跳转；倍速框跟着房间走（房间里 2 倍速时框里
  也是 2 倍，房间倍速不在选项里则保持原样不变空框）；
  另外还卡住「输入框/滑块里不吃快捷键、复选框上照常生效」「自动重复不算新按键」
  「哈希没对上房间时方向键不许跳」。
