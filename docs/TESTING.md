...
- `check-ui.mjs`：`app.js` 用到的 id/class 必须都在 `index.html` 里，且 id 不重复
  ...
  必须是浮在画面底部的绝对定位（不吃画面高度）、必须带 `transform: translateZ(0)`
  （视频自成合成层时浮层会被画到下面，表现是「点得到、看不见」）、淡出态必须有
  ...
  （不依赖事件目标），并保留 `?debug=1` 调试面板；`.stage.ctl-docked` 的停靠样式
...
- `check-fullscreen.mjs`：同样加载真实的 `web/app.js`，用假定时器推进时间，断言全屏控制条
  「点全屏时给 `.stage` 加 `.is-fs` → 指针按坐标落在画面里才浮现（画面外晃动不唤醒）→
  静止 2.6 秒淡出 → 鼠标停在控制条上或正拖进度条时不淡出 → 暂停时一直留着、续播后重新计时 →
  退出全屏摘掉 `.is-fs`，窗口模式下同样是这一套」，外加「顶栏切到停靠后一直可见、
  切回浮层照常淡出」。
