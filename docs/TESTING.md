...
- `check-ui.mjs`：`app.js` 用到的 id/class 必须都在 `index.html` 里，且 id 不重复
  ...
  控制条必须放在 `.video-wrap` 里面、必须是浮在画面底部的绝对定位（不吃画面高度）、
  淡出态必须有 `opacity:0 + pointer-events:none`、全屏布局必须由 `.is-fs` 类驱动、
  全屏元素必须是 `.stage`，而且 **CSS 里不许出现 `:-webkit-full-screen` / `:-moz-full-screen`**
  （Firefox 不认带前缀的全屏伪类，而选择器列表里只要有一个不认识的，整条规则会被整体丢掉——
  这正是「全屏后控制条不浮现」那次线上问题的根因）。
...
- `check-fullscreen.mjs`：同样加载真实的 `web/app.js`，用假定时器推进时间，断言全屏控制条
  「点全屏时给 `.stage` 加 `.is-fs` → 鼠标在画面上动/触摸/按键就浮现 → 静止 2.6 秒淡出 →
  鼠标停在控制条上或正拖进度条时不淡出 → 暂停时一直留着、续播后重新计时 →
  退出全屏摘掉 `.is-fs`，窗口模式下同样是这一套」。
