// 校验 web/rtest_hash.wasm：与 Node 的 crypto 以及主程序 CLI 算出的 SHA-256 是否完全一致。
...
const BIN = process.argv[2] || 'target/release/rtest';
const wasmPath = new URL('../web/rtest_hash.wasm', import.meta.url);
...
const ABI = 2;
check(ex.rtest_version() === ABI, `wasm ABI 版本为 ${ABI}（实际 ${ex.rtest_version()}）`);
...
const STAGE = 1 << 20;
const stagePtr = ex.rtest_alloc(STAGE);
...
try {
  ...
  const wasmSample = toHex(ex.rtest_finish());
